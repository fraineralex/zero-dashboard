...
export function MetricCard({ props }: { props: { label: string; value: string; delta?: string; tone?: string; helper?: string; action?: string; span?: string } }) {
  ...
  const content = (
    <>
      ...
      <div className="metric-foot">{props.delta ? <span className={cn("metric-delta", `tone-${tone}`)}><Icon size={13} />{props.delta}</span> : null}<span>{props.helper}</span></div>
    </>
  );
...
export function BarChartCard({ props }: { props: BaseCardProps & { data: ChartRow[]; xKey: string; series: Series[]; format?: string; horizontal?: boolean } }) {
  const firstSeries = props.series[0];
  const categoryWidth = props.horizontal ? Math.min(190, Math.max(128, ...props.data.map((row) => String(row[props.xKey] ?? "").length * 6.6 + 12))) : 0;
  const categoryLength = Math.max(16, Math.floor((categoryWidth - 12) / 6.6));
  return (
    <CardFrame {...props}>
      <ChartContainer config={chartConfig(props.series)} className="chart-standard" aria-label={`${props.title} chart`}>
        <BarChart data={props.data} layout={props.horizontal ? "vertical" : "horizontal"} margin={{ top: props.data.length <= 6 ? 23 : 12, right: 10, bottom: 0, left: props.horizontal ? 8 : 0 }}>
          ...
          {props.horizontal ? <YAxis dataKey={props.xKey} type="category" tickLine={false} axisLine={false} width={categoryWidth} tickFormatter={(value: string) => value.length > categoryLength ? `${value.slice(0, categoryLength - 1)}…` : value} /> : <XAxis dataKey={props.xKey} tickLine={false} axisLine={false} tickMargin={10} />}
...
export function DataTable({ props }: { props: BaseCardProps & { data: Record<string, unknown>[]; columns: { key: string; label: string; format?: string }[]; rowAction?: string; currency?: string; total?: number } }) {
  ...
  const filteredRows = props.currency === "DOP" && query ? props.data.filter((row) => Object.values(row).some((value) => String(value).toLocaleLowerCase("es-DO").includes(query.toLocaleLowerCase("es-DO")))) : props.data;
  const visibleRows = sort ? [...filteredRows].sort((left, right) => {
    const a = left[sort.key];
    const b = right[sort.key];
    const comparison = typeof a === "number" && typeof b === "number" ? a - b : String(a ?? "").localeCompare(String(b ?? ""), "es-DO", { numeric: true });
    return sort.descending ? -comparison : comparison;
  }) : filteredRows;
  const pageSize = 10;
  const pageCount = Math.max(1, Math.ceil(visibleRows.length / pageSize));
  const activePage = Math.min(page, pageCount - 1);
  const displayedRows = props.currency === "DOP" ? visibleRows.slice(activePage * pageSize, (activePage + 1) * pageSize) : visibleRows;
  const toggleSort = (key: string) => { setSort((current) => current?.key === key ? { key, descending: !current.descending } : { key, descending: false }); setPage(0); };
  ...
  return (
    <CardFrame {...props} className="table-card">
      {props.currency === "DOP" ? <div className="erp-table-toolbar"><span><span className="erp-source-dot" />{visibleRows.length} de {props.data.length} registros · ERP demo</span><label><Search size={14} /><span className="sr-only">Filtrar registros</span><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(0); }} placeholder="Filtrar esta vista…" /></label><select className="erp-mobile-sort" aria-label="Ordenar registros" value={sort ? `${sort.key}:${sort.descending ? "desc" : "asc"}` : "default"} onChange={(event) => { const [key, direction] = event.target.value.split(":"); setSort(key === "default" ? null : { key, descending: direction === "desc" }); setPage(0); }}><option value="default">Orden original</option>{props.columns.flatMap((column) => [<option key={`${column.key}:asc`} value={`${column.key}:asc`}>{column.label} ↑</option>, <option key={`${column.key}:desc`} value={`${column.key}:desc`}>{column.label} ↓</option>])}</select></div> : null}
      <div className="table-scroll"><Table className={props.currency === "DOP" ? "erp-table" : undefined}><TableHeader><TableRow>{props.columns.map((column) => <TableHead key={column.key} aria-sort={props.currency === "DOP" && sort?.key === column.key ? sort.descending ? "descending" : "ascending" : "none"}>{props.currency === "DOP" ? <button type="button" className="erp-sort-button" onClick={() => toggleSort(column.key)} aria-label={`Ordenar por ${column.label}`}>{column.label}<ArrowUpDown size={11} /></button> : column.label}</TableHead>)}</TableRow></TableHeader><TableBody>{displayedRows.map((row, index) => <TableRow key={String(row.id ?? row.customer ?? index)} className={props.rowAction ? "clickable-row" : undefined} onClick={props.rowAction && String(row.id ?? "").includes("acme") ? () => openCustomer("acme") : undefined}>{props.columns.map((column) => <TableCell key={column.key} className={cn(column.format === "dop" && "erp-amount", column.key === "status" && "erp-status-cell")}>{column.key === "status" && props.currency === "DOP" ? <span className="erp-status">{String(row[column.key])}</span> : column.format === "dop" ? dop.format(Number(row[column.key])) : formatValue(row[column.key] as string | number, column.format)}{column.key === "customer" && String(row.id ?? "").includes("acme") ? <ChevronRight size={13} /> : null}</TableCell>)}</TableRow>)}{visibleRows.length === 0 ? <TableRow><TableCell colSpan={props.columns.length}>No hay coincidencias en esta vista.</TableCell></TableRow> : null}</TableBody></Table></div>
      {props.currency === "DOP" ? <div className="erp-mobile-records">{displayedRows.map((row, index) => <article key={String(row.id ?? index)} className="erp-mobile-record"><div className="erp-mobile-record-head"><strong>{String(row.name ?? row.id ?? row.employee ?? row.product ?? index + 1)}</strong>{typeof row.amount === "number" || typeof row.net === "number" ? <b>{dop.format(Number(row.amount ?? row.net))}</b> : null}</div><div className="erp-mobile-fields">{props.columns.filter((column) => column.key !== "id" && column.key !== "amount" && column.key !== "net").map((column) => <div key={column.key}><span>{column.label}</span><strong>{column.format === "dop" ? dop.format(Number(row[column.key])) : String(row[column.key] ?? "—")}</strong></div>)}</div></article>)}{visibleRows.length === 0 ? <p className="erp-mobile-empty">No hay coincidencias en esta vista.</p> : null}</div> : null}
      {props.currency === "DOP" && pageCount > 1 ? <div className="erp-table-pagination"><span>Página {activePage + 1} de {pageCount}</span><div><button type="button" onClick={() => setPage(Math.max(0, activePage - 1))} disabled={activePage === 0}>Anterior</button><button type="button" onClick={() => setPage(Math.min(pageCount - 1, activePage + 1))} disabled={activePage >= pageCount - 1}>Siguiente</button></div></div> : null}
